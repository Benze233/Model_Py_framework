...
class AutoEncoder(nn.Module):
    ...
    def train_Model(self, train_in: np.array, train_out: np.array, MaxEpoch: int, batchsize: int, learning_rate,
        ...
        train_in = torch.from_numpy(train_in.T).to(device, dtype=torch.float32)
        train_out = torch.from_numpy(train_out.T).to(device, dtype=torch.float32)
    ...
    def save(self, base_name="Autoencoder_model"):
    ...
    def load(base_name="Autoencoder_model"):
        ...
        AE_model = AutoEncoder(loaded_dict['stateshape'], loaded_dict['hiddenshape'])
